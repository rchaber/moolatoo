...
BaseDocumentManager provides some common utilities, and the Item subclass
adds some Item-document-specific helper methods.
"""
...
from models import item_config
...
class Store(BaseDocumentManager):
    ...
    _INDEX_NAME = item_config.STORE_INDEX_NAME
    STORE_NAME = 'store_name'
    STORE_LOCATION = 'store_location'
    STORE_ADDRESS = 'store_address'


class Item(BaseDocumentManager):
    """Provides helper methods to manage Item documents.  All Item documents
    built using these methods will include a core set of fields (see the
    _buildCoreItemFields method).  We use the given item id (the Item
    ...
    decoupled-- but using the item id as the doc id allows a document to be
    reindexed given its item info, without having to fetch the
    existing document."""

    _INDEX_NAME = item_config.ITEM_INDEX_NAME

    # 'core' item document field names
    IID = 'iid'
    ...
    _SORT_OPTIONS = [
                ...
                [ESTIMATEDVALUE, 'est. value', search.SortExpression(
                        # other examples:
                        # expression='max(estimatedvalue, 14.99)'
                        ...
                        # expression='estimatedvalue * _score'
                        expression=ESTIMATEDVALUE,
                ...
                [ITEM_NAME, 'item name', search.SortExpression(
    ...
    def _buildSortMenu(cls):
        """Build the default set of sort options used for Item search.
    ...
    @classmethod
    def getDocFromIid(cls, iid):
        """Given a iid, get its doc. We're using the iid as the doc id, so we can
        do this via a direct fetch."""
        return cls.getDoc(iid)

    @classmethod
    def removeItemDocByIid(cls, iid):
        """Given a doc's iid, remove the doc matching it from the item
        index."""
        cls.removeDocById(iid)

    @classmethod
    def updateRatingInDoc(cls, doc_id, avg_rating):
        # get the associated doc from the doc id in the item entity
    ...
    def updateRatingsInfo(cls, doc_id, avg_rating):
        """Given a models.Item entity, update and reindex the associated
        document with the item entity's current average rating. """
...
# 'accessor' convenience methods
    ...
    def getIID(self):
        """Get the value of the 'iid' field of a Item doc."""
        return self.getFieldVal(self.IID)

    def getName(self):
        """Get the value of the 'name' field of a Item doc."""
        return self.getFieldVal(self.ITEM_NAME)

    def getDescription(self):
        """Get the value of the 'description' field of a Item doc."""
        return self.getFieldVal(self.DESCRIPTION)

    def getCategory(self):
        """Get the value of the 'cat' field of a Item doc."""
        return self.getFieldVal(self.CATEGORY)

    def setCategory(self, cat):
        """Set the value of the 'cat' (category) field of a Item doc."""
        return self.setFirstField(search.NumberField(name=self.CATEGORY, value=cat))

    def getAvgRating(self):
        """Get the value of the 'ar' (average rating) field of a Item doc."""
        return self.getFieldVal(self.AVG_RATING)

    def setAvgRating(self, ar):
        """Set the value of the 'ar' field of a Item doc."""
        return self.setFirstField(search.NumberField(name=self.AVG_RATING, value=ar))

    def getEstimatedvalue(self):
        """Get the value of the 'est. value' field of a Item doc."""
        return self.getFieldVal(self.ESTIMATEDVALUE)
    ...
    def generateRatingsLinks(cls, query, phash):
        ...
        if not ratings_buckets:
            return None
        rlist = []
        for k in range(item_config.RATING_MIN, item_config.RATING_MAX + 1):
            try:
                v = ratings_buckets[k]
            except KeyError:
                return
    ...
    @classmethod
    def _buildCoreItemFields(
            cls, iid, name, description, category, category_name, estimatedvalue):
        """Construct a 'core' document field list for the fields common to all
        Items. The various categories (as defined in the file 'categories.py'),
        may add additional specialized fields; these will be appended to this
        core list. (see _buildItemFields)."""
        fields = [search.TextField(name=cls.IID, value=iid),
                            # The 'updated' field is always set to the current date.
                            search.DateField(name=cls.UPDATED,
                                    value=datetime.datetime.now().date()),
                            search.TextField(name=cls.ITEM_NAME, value=name),
                            ...
                            search.NumberField(name=cls.ESTIMATEDVALUE, value=estimatedvalue)
                         ]
        return fields

    @classmethod
    def _buildItemFields(cls, iid=None, category=None, name=None,
            description=None, category_name=None, estimatedvalue=None, **params):
        """Build all the additional non-core fields for a document of the given
        item type (category), using the given params dict, and the
        already-constructed list of 'core' fields.  All such additional
        category-specific fields are treated as required.
        """

        fields = cls._buildCoreItemFields(
                iid, name, description, category, category_name, estimatedvalue)
        # get the specification of additional (non-'core') fields for this category
        pdict = categories.item_dict.get(category_name)
        if pdict:
            # for all fields
            ...
            logging.warn(
                    'item field information not found for category name %s',
                    params['category_name'])
        return fields

    @classmethod
    def _createDocument(
            cls, iid=None, category=None, name=None, description=None,
            category_name=None, estimatedvalue=None, **params):
        """Create a Document object from given params."""
        # check for the fields that are always required.
        if iid and category and name:
            # First, check that the given iid has only visible ascii characters,
            # and does not contain whitespace.  The iid will be used as the doc_id,
            # which has these requirements.
            if not cls.isValidDocId(iid):
                raise errors.OperationFailedError("Illegal iid %s" % iid)
            # construct the document fields from the params
            resfields = cls._buildItemFields(
                    iid=iid, category=category, name=name,
                    description=description,
                    category_name=category_name, estimatedvalue=estimatedvalue, **params)
            # build and index the document.  Use the iid (item id) as the doc id.
            # (If we did not do this, and left the doc_id unspecified, an id would be
            # auto-generated.)
            d = search.Document(doc_id=iid, fields=resfields)
            return d
        else:
    ...
    def _normalizeParams(cls, params):
        """Normalize the submitted params for building a item."""

        params = copy.deepcopy(params)
        try:
            params['iid'] = params['iid'].strip()
            params['name'] = params['name'].strip()
            params['category_name'] = params['category']
            params['category'] = params['category']
            try:
                params['estimatedvalue'] = float(params['estimatedvalue'])
            except ValueError:
                error_message = 'bad estimatedvalue value: %s' % params['estimatedvalue']
                logging.error(error_message)
                raise errors.OperationFailedError(error_message)
            return params
    ...
    @classmethod
    def buildItemBatch(cls, rows):
        """Build item documents and their related datastore entities, in batch,
        given a list of params dicts.  Should be used for new items, as does not
        handle updates of existing item entities. This method does not require
        that the doc ids be tied to the item ids, and obtains the doc ids from
        ...
        for row in rows:
            try:
                ...
                docs.append(doc)
                # create item entity, sans doc_id
                dbp = models1.Item(
    ...
    @classmethod
    def buildItem(cls, params):
        """Create/update a item document and its related datastore entity.  The
        item id and the field values are taken from the params dict.
        """
        ...
        # were not the case.
        curr_doc = cls.getDocFromIid(params['iid'])
        ...
        logging.debug('got new doc id %s for item: %s', doc_id, params['iid'])

        # now update the entity
        def _tx():
            # Check whether the item entity exists. If so, we want to update
            # from the params, but preserve its ratings-related info.
            ite = models1.Item.get_by_id(params['iid'])
            if ite:     # update
                ite.update_core(params, doc_id)
            else:   # create new entity
                ite = models1.Item.create(params, doc_id)
            ite.put()
            return ite
        ite = ndb.transaction(_tx)
        logging.debug('ite: %s', ite)
        return ite
